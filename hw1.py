...
def my_relu(in_value):
    ...
    out_value = tf.maximum(c1, in_value)
    return out_value
...
def my_perceptron(x):
    ...
    w = tf.get_variable(name='w', shape=[x], initializer=tf.constant_initializer(1))
    out = my_relu(tf.tensordot(i, w, 1))
...
def onelayer(X, Y, layersize=10):
    ...
    w = tf.Variable(tf.random_normal(shape=[int(X.shape[1]), layersize], stddev=0.1))
...
def twolayer(X, Y, hiddensize=30, outputsize=10):
    ...
    w1 = tf.Variable(tf.random_normal(shape=[int(X.shape[1]), hiddensize], stddev=0.1))
    b1 = tf.Variable(tf.zeros(shape=[hiddensize]))
    logits1 = tf.matmul(X, w1) + b1
    preds1 = my_relu(logits1)

    w2 = tf.Variable(tf.random_normal(shape=[hiddensize, outputsize], stddev=0.1))
    b2 = tf.Variable(tf.zeros(shape=[outputsize]))
...
def convnet(X, Y, convlayer_sizes=[10, 10], \
    ...
    two_dim = tf.reshape(conv2, [-1, int(conv2.shape[1]) * int(conv2.shape[2]) * int(conv2.shape[3])])
    w, b, logits, preds, batch_xentropy, batch_loss = onelayer(two_dim, Y)
